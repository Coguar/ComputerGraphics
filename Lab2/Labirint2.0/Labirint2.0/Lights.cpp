...
CSpotlightSource::CSpotlightSource(unsigned index)
...
void CSpotlightSource::SetDirection(glm::vec3 const & dir)
{
	m_direction.x = dir.x;
	m_direction.y = dir.y;
	m_direction.z = dir.z;
...
void CSpotlightSource::Setup() const
{
	CPositionLightSource::Setup();
	glLightfv(GetIndex(), GL_SPOT_DIRECTION, glm::value_ptr(m_direction));
	glLightf(GetIndex(), GL_SPOT_EXPONENT, 0.f);
	glLightf(GetIndex(), GL_SPOT_CUTOFF, 25.0f);
}
