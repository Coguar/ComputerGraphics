...
CLine3D::CLine3D(float startX, float endX, float step)
{
	Tesselate(startX, endX, step);
}

void CLine3D::Draw() const
{
	DoWithBindedArrays(m_vertices, [this] {
		glDrawElements(GL_TRIANGLE_STRIP, GLsizei(m_indicies.size()),
			GL_UNSIGNED_INT, m_indicies.data());
...
void CLine3D::Tesselate(float startX, float endX, float step)
{
	m_vertices.clear();
	size_t steps = size_t(std::abs(endX - startX) / step);
	std::cout << startX << std::endl;

	for (size_t i = 0; i <= steps; ++i)
	{
		SVertexP3N vertex;
		vertex.position = { startX + i * step, 0.f, 0.f };
		vertex.normal = glm::vec3(0.f, 0.f, 1.f);
		m_vertices.push_back(vertex);
		m_indicies.push_back(i);

		vertex.position = { startX + i * step, 0.1f, 0.f };
		vertex.normal = glm::vec3(0.f, 0.f, 1.f);
		m_vertices.push_back(vertex);
		m_indicies.push_back(i);
	}
	std::cout << startX + steps * step << std::endl;
}
