...
glm::vec3 GetPositionOnTor(float u, float v)
{
	return{ (1.0f + 0.5f * cos(u)) * cos(v),
			(1.0f + 0.5f * cos(u)) * sin(v),
			0.5 * sin(u) };
}
...
CTor::CTor(unsigned slices, unsigned stacks)
{
	Tesselate(slices, stacks);
}

void CTor::Draw() const
{
	m_mesh.Draw();
}

void CTor::Tesselate(unsigned slices, unsigned stacks)
{
	assert((slices >= MIN_PRECISION) && (stacks >= MIN_PRECISION));
	...
	for (float u = 0; u <= 2 * M_PI; u += 0.1)
	{
		...
		{
			SVertexP3NT2 vertex;
			vertex.position = GetPositionOnTor(u, v);
...
CField::CField(unsigned slices, unsigned stacks)
{
	Tesselate(slices, stacks);
}
...
void CField::Tesselate(unsigned slices, unsigned stacks)
{
	assert((slices >= MIN_PRECISION) && (stacks >= MIN_PRECISION));
	...
	for (unsigned ci = 0; ci < slices; ++ci)
	{
		const float u = float(ci) / float(slices - 1) * 2 * M_PI - M_PI;
		for (unsigned ri = 0; ri < stacks; ++ri)
		{
			const float v = float(ri) / float(stacks - 1) * 2 * M_PI - M_PI;

			SVertexP3NT2 vertex;
			vertex.position = glm::vec3(u, -1, v);
