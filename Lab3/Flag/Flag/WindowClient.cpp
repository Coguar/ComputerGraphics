...
namespace
{
const glm::vec2 QUAD_TOPLEFT = { -300, -150 };
const glm::vec2 QUAD_SIZE = { 600, 300 };
    ...
    , m_quadObj(QUAD_TOPLEFT, QUAD_SIZE)
{
	...
	const std::string flagShader = CFilesystemUtils::LoadFileAsString("res/Russian.frag");
	...
	m_programSSSR.CompileShader(vertexShader, ShaderType::Vertex);
	m_programSSSR.CompileShader(flagShader, ShaderType::Fragment);
	m_programSSSR.Link();

    m_programQueue = { &m_programPicture, &m_programCheckers, &m_programSSSR };
}
