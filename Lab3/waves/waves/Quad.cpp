...
bool CQuad::HitTest(glm::vec2 const & position)
{
	return (position.x > m_rect.m_topLeft.x && position.x < m_rect.m_topLeft.x + m_rect.m_size.x)
					&& (position.y > m_rect.m_topLeft.y && position.y < m_rect.m_topLeft.y + m_rect.m_size.y);
}
